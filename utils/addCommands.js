...
devices.findOne({imei:'359710046031556'},function(err,device){
    vehicles.findOne({deviceId:device._id},function(err, vehicle){
        ...
        var turnOn = new deviceMessagesModel({
            vehicle : vehicle._id,
            command: "588bf6e04968277ba9cab0a8", // 588bf6e04968277ba9cab0a6 shutdown 588bf6e04968277ba9cab0a8 turnon
            user : "5841fa4d129de87a2fd567dc" // 5841fa4d129de87a2fd567dc hexenon
        });
        setTimeout(function(){
            turnOn.save(function(err){console.log(err ? 'Error. ' + err.toString() : 'Command SENT');});
        },15000);
